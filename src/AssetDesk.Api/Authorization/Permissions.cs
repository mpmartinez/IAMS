...
public static class Permissions
{
    ...
    /// The "iams:" prefix predates the AssetDesk rename and stays. These strings are
    /// persisted in RolePermissions and issued as claims, so changing the prefix requires a
    /// data migration that rewrites existing rows - not a rename.
    ...
    public static IReadOnlyList<string> DefaultsFor(string roleName) => roleName switch
    {
        ...
        // copying but genuinely blocks that cast, matching what
        // ClaimsPrincipalExtensions.GrantableKeys already does for the same array.
        //
